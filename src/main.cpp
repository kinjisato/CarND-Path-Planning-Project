      ...
      if (s != "") {
        ...
        if (event == "telemetry") {
          // j[1] is the data JSON object
            ...
            // check which lane my car is running
            /*s
            if (car_d <= 4){
                lane = 0;
            ...
            */
            ...
            cout << "car_vel :" << car_vel << endl;
            // cout << "target_speed :" << target_speed << endl;
            */
            
            // create a list of widely spaced(x,y) waypoints, evenly spaced at 30m
            // Later we will interporate these waypoints with a spline and fill it
            vector<double> ptsx;
            vector<double> ptsy;
            
            // reference x,y yaw states
            // either we will reference the starting point as where the car is at the previous paths and points
            double ref_x = car_x;
            double ref_y = car_y;
            ...
            // Calculate how to break up spline points so that we travel at our desired refefence velocity
            ...
            // Fill up to the rest of our path planner after filling it with previous poits, here we will always output 50 pints.
            for (int i=0; i<=50-previous_path_x.size(); i++){
                ...
                double N = (target_dist/(.02*ref_vel/2.24)); // 2.24 for mile to meter per second
            ...
            /*
            // 20180930
            // Project lecture : 2. Getting started
            double dist_inc = 0.3; // 0.5
            for(int i = 0; i < 50; i++)
            {
                // Frenet cordinates
                double next_s = car_s + (i + 1) * dist_inc;
                double next_d = 6;  // 4 + 4/2;
                
                // getXY cordineates
                vector<double> xy = getXY(next_s, next_d, map_waypoints_s, map_waypoints_x, map_waypoints_y);
                
                // go straight at 50mph
